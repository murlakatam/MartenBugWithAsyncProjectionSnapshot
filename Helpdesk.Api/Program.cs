...
builder.Services
    ...
    .AddMarten(sp =>
    {
        ...
        options.Projections.Snapshot<IncidentDetailsSnapshotAsyncProjection>(SnapshotLifecycle.Async);
...
builder.Services
    ...
    .ConfigureMarten((sp, opts) => { opts.Logger(new MartenLogger(sp.GetRequiredService<ILogger<MartenLogger>>())); })
...
incidents.MapGet("{incidentId:guid}/aggregate",
    (HttpContext context, IQuerySession querySession, Guid incidentId) =>
    {
        var aggregate = querySession.Query<IncidentDetailsSnapshotAsyncProjection>().Where(i => i.Id == incidentId);
        return querySession.Json.WriteById<IncidentDetailsSnapshotAsyncProjection>(incidentId, context);
    }
);
